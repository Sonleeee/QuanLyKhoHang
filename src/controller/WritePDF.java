    ...
    public void writePhieuNhap(String mapn) {
        ...
        try {
            ...
            for (ChiTietPhieu ctpn : ChiTietPhieuNhapDAO.getInstance().selectAll(mapn)) {
                Hang mt = HangDAO.getInstance().selectById(ctpn.getMaHang());
                pdfTable.addCell(new PdfPCell(new Phrase(ctpn.getMaHang(), fontData)));
                pdfTable.addCell(new PdfPCell(new Phrase(mt.getTenHang(), fontData)));
